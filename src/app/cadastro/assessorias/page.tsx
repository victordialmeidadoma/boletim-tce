...
import { Plus, Trash2, Upload, CheckCircle, Briefcase, Loader2 } from "lucide-react";
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-8">
      ...
      <div className="bg-white rounded-2xl shadow-card p-5 mb-6">
        <h2 className="text-sm font-semibold text-ink-800 mb-4">Nova assessoria</h2>
        ...
        <div className="mb-4">
          ...
          <button onClick={() => logoRef.current?.click()}
            className={cn("w-full py-2.5 rounded-xl border border-dashed text-sm transition-colors flex items-center justify-center gap-2",
              logoFile ? "border-emerald-300 bg-emerald-50 text-emerald-700" : "border-ink-300 text-ink-500 hover:border-brand-300 hover:text-brand-600 hover:bg-brand-50/50")}>
            {logoFile ? <><CheckCircle className="w-4 h-4" />{logoFile.name}</> : <><Upload className="w-4 h-4" />Selecionar logo</>}
          </button>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl shadow-card overflow-hidden">
          {assessorias.length === 0 ? (
            ...
            <div key={a.id} className={cn("flex items-center gap-3 px-5 py-3.5", i < assessorias.length - 1 && "border-b border-ink-100")}>
